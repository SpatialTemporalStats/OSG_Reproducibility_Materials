...
Figure 2 illustrates Slepian bases and eigenvalues in the Slepian concentration problem. Please refer to the "Wrapper.R" file for its reproducibility command. The total computational time is approximately 2.78 seconds. Before running the code, please download the required Slepian bases and eigenvalues for demonstration from https://zenodo.org/records/16655516 to facilitate reproduction of Figure 2. In addition, MATLAB code for reproducing the bases independently is available on the same website. This code applies the method proposed by [Bates et al. (2017)](https://dl.acm.org/doi/abs/10.1109/TSP.2017.2712122) to the ARP. For more detailed code and computational time, please refer to the file "Slepian_Demonstration.R" in the sub-repository "Slepian_Demonstration" and the website mentioned above. 
