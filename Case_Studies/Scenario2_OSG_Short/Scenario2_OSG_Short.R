################################################################################################
# OSG-Short in Scenario 2: Construction, Emulation, and Evaluation                             #
################################################################################################
# ! Note. In this file, we have to store cumulative estimates^[b] at all b (data block index), 
#         b=1,...,B, to plot Figure 8. The Algorithm 3 (our OSG algorithm) itself just need to 
#         store cumulative estimates^[b] at the latest b. To obtain the real algorithm, please 
#         remove lines 82, 101-102, 124-127, 196-197, 225-226, 230-233
...
B=517           # Number of data blocks -1
T0=31*8         # Length of initial data block
Tt=7*8          # Length of subsequent data blocks


# T1=proc.time()[[3]]
####### Part I. Develop the OSG-Short  #########
...
writeMat(here("Case_Studies/Scenario2_OSG_Short","TGHpara_u_all.mat"),TGHparauall=TGHpara.u.all)
writeMat(here("Case_Studies/Scenario2_OSG_Short","TGHpara_v_all.mat"),TGHparavall=TGHpara.v.all)
writeMat(here("Case_Studies/Scenario2_OSG_Short","Phihat_online.mat"),Phihatonline=Phi.hat.all[c(1,B+1),,])
writeMat(here("Case_Studies/Scenario2_OSG_Short","K_online.mat"),Konline=K.all[c(1,B+1),,])
# T2=proc.time()[[3]]
# T2-T1=2278.658
...
RFD.Phi=matrix(0,B+1,2)
RFD.K=rep(0,B+1)
for(b in 1:(B+1)){
  RFD.Phi[b,1]=sqrt(sum((Phi.hat.all[b,1:(2*Q.sl),]-Phi.hat.full[1:(2*Q.sl),])^2))/sqrt(sum(Phi.hat.full[1:(2*Q.sl),]^2))
  RFD.Phi[b,2]=sqrt(sum((Phi.hat.all[b,-(1:(2*Q.sl)),]-Phi.hat.full[-(1:(2*Q.sl)),])^2))/sqrt(sum(Phi.hat.full[-(1:(2*Q.sl)),]^2))
  RFD.K[b]=sqrt(sum((K.all[b,,]/R/(T0+(b-1)*Tt-b*P)-K.full)^2))/sqrt(sum(K.full^2))
}
write.csv(cbind(RFD.Phi,RFD.K),here("Case_Studies/Scenario2_OSG_Short","RFDs_OSG_Short.csv"))
# t2=proc.time()[[3]]
# t2-t1=35.749



####### Part II. Generate emulations using OSG-Short by Algorithm S2 #########
# t1=proc.time()[[3]]
...
Iuq.u.online=get.IUQ(Gen.Dat.U.online,Windu.ARP)
Iuq.v.online=get.IUQ(Gen.Dat.V.online,Windv.ARP)
write.csv(Iuq.u.online,here("Case_Studies/Scenario2_OSG_Short","Iuq_u_online_2.csv"))
write.csv(Iuq.v.online,here("Case_Studies/Scenario2_OSG_Short","Iuq_v_online_2.csv"))
...
Ibc.online=get.IBC(Genu.rsd,Genv.rsd,Windu.rsd,Windv.rsd)
write.csv(Ibc.online,here("Case_Studies/Scenario2_OSG_Short","Ibc_online_2.csv"))


### Prepare for calculating Itc1 and Itc2 by calculating temporal correlations at lag=1 and 2
Itc1.u.online=get.ITC1(Genu.rsd,Windu.rsd)
Itc1.v.online=get.ITC1(Genv.rsd,Windv.rsd)
Itc2.u.online=get.ITC2(Genu.rsd,Windu.rsd)
Itc2.v.online=get.ITC2(Genv.rsd,Windv.rsd)
write.csv(Itc1.u.online,here("Case_Studies/Scenario2_OSG_Short","Itc1_u_online_2.csv"))
write.csv(Itc1.v.online,here("Case_Studies/Scenario2_OSG_Short","Itc1_v_online_2.csv"))
write.csv(Itc2.u.online,here("Case_Studies/Scenario2_OSG_Short","Itc2_u_online_2.csv"))
write.csv(Itc2.v.online,here("Case_Studies/Scenario2_OSG_Short","Itc2_v_online_2.csv"))
...
write.csv(cbind(NCPGT.emu,NCPGT.dat),here("Case_Studies/Scenario2_OSG_Short","Itp_online_2.csv"))


### Calculate Iwdt and Iwds for marginal distribution
Iwdt.u.online=get.IWDT(Gen.Dat.U.online,Windu.ARP)
Iwdt.v.online=get.IWDT(Gen.Dat.V.online,Windv.ARP)
Iwds.u.online=get.IWDS(Gen.Dat.U.online,Windu.ARP)
Iwds.v.online=get.IWDS(Gen.Dat.V.online,Windv.ARP)
write.csv(Iwdt.u.online,here("Case_Studies/Scenario2_OSG_Short","Iwdt_u_online_2.csv"))
write.csv(Iwdt.v.online,here("Case_Studies/Scenario2_OSG_Short","Iwdt_v_online_2.csv"))
write.csv(Iwds.u.online,here("Case_Studies/Scenario2_OSG_Short","Iwds_u_online_2.csv"))
write.csv(Iwds.v.online,here("Case_Studies/Scenario2_OSG_Short","Iwds_v_online_2.csv"))


### Calculate Imd for mean shift
Imd.online=c(apply(sqrt((Windu.EnMean-GenDu.EnMean)^2),1,mean),
               apply(sqrt((Windv.EnMean-GenDv.EnMean)^2),1,mean))
write.csv(Imd.online,here("Case_Studies/Scenario2_OSG_Short","Imd_online_2.csv"))
...
Isdd.online=c(apply(sqrt((Windu.EnSD-GenDu.EnSD)^2),1,mean),
                apply(sqrt((Windv.EnSD-GenDv.EnSD)^2),1,mean))
write.csv(Isdd.online,here("Case_Studies/Scenario2_OSG_Short","Isdd_online_2.csv"))
...
I75qd.online=c(apply(sqrt((Windu.En75-GenDu.En75)^2),1,mean),apply(sqrt((Windv.En75-GenDv.En75)^2),1,mean))
write.csv(I75qd.online,here("Case_Studies/Scenario2_OSG_Short","I75qd_online_2.csv"))
...
I25qd.online=c(apply(sqrt((Windu.En25-GenDu.En25)^2),1,mean),apply(sqrt((Windv.En25-GenDv.En25)^2),1,mean))
write.csv(I25qd.online,here("Case_Studies/Scenario2_OSG_Short","I25qd_online_2.csv"))
