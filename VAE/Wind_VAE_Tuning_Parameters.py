...
# -*- coding: utf-8 -*-
#%% Document
# Two-Stage Approach: we assume that spatial data (or images) at different time points are independent, first modeling the spatial structure using a variational autoencoder (VAE), and then modeling the temporal dynamics of the latent variables across time.
# This file tunes the VAE architecture by shuffling spatial images across different time points, splitting the data into training and validation sets, and evaluating model performance using the validation reconstruction loss, validation total loss.
# Tunning follows the order as below:
# 1) latent dimension: 8, 16, 32, 64, 128 --> 64
# 2) decoder width: 16, 32, 64, 96, 128 --> 96
# 3) beta for balancing regularization and KL: 0.4, 0.5, 0.7, 1, 3 --> 0.7
# 4) learning rate: 1e-3, 5e-4, 1e-4, 5e-5 --> 1e-4
# 5) number of filters: (32,64), (64, 128), (64,256) --> (32,64)
# 6) dense: 64, 128, 256, 512, 1024, 1280 --> 1024
# 7) batch size: 32, 64, 128 --> 32
...
N=Wind_ARP.shape[0]
np.random.seed(123)
perm=np.random.permutation(N)                   # shuffle the dataset
x_train=Wind_ARP[perm[:int(0.5*N)]]             # split the data 
x_val=Wind_ARP[perm[int(0.5*N):]]
...
#%% Train the VAE and test on validation set
...
vae.fit(x_train,epochs=30,batch_size=32) # train the vae
z_mean_val, z_log_var_val, z_val =vae.encoder.predict(x_val, verbose=0)
x_val_recon=vae.decoder.predict(z_val,verbose=0)
val_recon_loss=tf.reduce_mean(tf.reduce_sum(tf.square(x_val-x_val_recon),axis=(1,2,3))).numpy()
val_kl_loss=tf.reduce_mean(tf.reduce_sum(-0.5*(1+z_log_var_val-tf.square(z_mean_val)-tf.exp(z_log_var_val)),axis=1)).numpy()
val_total_loss=val_recon_loss+0.7*val_kl_loss
print(val_recon_loss,val_kl_loss,val_total_loss)
